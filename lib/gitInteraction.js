...
import { splitIntoLines, strrpart } from './stringUtils.js';
...
export default class GitInteraction {
   ...
   * @returns {Promise.<Array.<string>>} File contents
   */
  async getCasefileContentLines(path, { beforeCommit } = {}) {
    const contentLines = [];
  ...
  async lineIntroduction(filePath, line, { commit, liveContent } = {}) {
    ...
    return this.runGitCommand('blame', {
      ...
      stdout: (chunk, endStream) => {
        const firstLine = splitIntoLines(chunk)[0];
  ...
  async findCurrentLinePosition(filePath, {commit, line}, content) {
    ...
    return new Promise((resolve, reject) => {
      this.runGitCommand('blame', {
        ...
        stdout: (chunk, endStream) => {
          for (const line of splitIntoLines(chunk)) {
            // See if *line* is describing the commit we need
