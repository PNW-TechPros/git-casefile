import GitInteraction, { ASSERT_ERROR, GitInterationError as ExtendedError, gitEmptyTree } from '../lib/gitInteraction';
...
describe('GitInteraction', () => {
  ...
  class GitMock {
    ...
    _stdioStreams(stdout) {
      ...
      if (stdout && stdout.write) {
        ...
        let bufferedOutput = '';
        const closeStdout = () => { result.stdout.destroy(); };
        const passToStdout = (content) => {
          try {
            stdout(content, closeStdout);
          } catch (e) {
            result.stdout.destroy(e);
          }
        }
        result.stdout.on('data', (data) => {
          ...
          if (niceLines) {
            passToStdout(niceLines);
          }
        });
        result.stdout.on('end', () => {
          if (bufferedOutput) {
            passToStdout(bufferedOutput);
          }
        });
      }
  ...
  describe('.prototype.fetchSharedCasefilesFromRemote()', function () {
    ...
    it(`throws TypeError if remoteName is undefined`, async function() {
      await expect(this.repo.fetchSharedCasefilesFromRemote())
        .is.rejectedWith(TypeError);
    });
    
    it(`throw TypeError if remoteName is null`, async function() {
      await expect(this.repo.fetchSharedCasefilesFromRemote())
        .is.rejectedWith(TypeError);
    });
  });
  ...
  describe('.prototype.getListOfCasefiles()', function () {
    ...
    it(`skips unknown tree entries`, async function() {
      const casefileName = 'groupA/casefile1';
      const instanceIds = [
        '22218950-279d-550d-b2c0-d776c50cc6a9',
        'ed421d07-97a9-5cb4-ba17-866e68ae5ce5',
      ...
      this.gitMock.expectCall(async ({ stdio: { stdout } }) => {
        await streamConsumed(stdout, () => {
          stdout.write(`100644 blob 05ceffd002e09e8cdc8db79bd37c52d66eb4e612\t${casefileName}/${instanceIds[0]}\0`);
          stdout.write(`777777 blob 7bff9859be4b3033d554dc86f85c80be4a4dc954\tweirdEntry1\0`);
          stdout.write(`100644 boom ccc53d5ac03d8d1d121f60a122475d869641ed28\tweirdEntry2\0`);
          stdout.write(`100644 blob 2fb8b967b5b10d97b3af6a7b41f06e8c74ff31ff\t${casefileName}/${instanceIds[1]}\0`);
        });
      }, { command: 'ls-tree', opts: expectedOpts, args: expectedArgs })
      const result = await this.repo.getListOfCasefiles();
  ...
  describe('.prototype.getCasefileAuthors()', function () {
    ...
    it('folds multiple entries for the same author together', async function () {
      ...
      const result = await this.repo.getCasefileAuthors(casefilePath);
      expect(result).to.deep.equal({
        path: casefilePath,
        authors: [ author ],
      });
  ...
  describe('.prototype.getCasefileContentLines()', function () {
    const casefilePath = 'a casefile/22218950-279d-550d-b2c0-d776c50cc6a9';
    const expectedGitCmd = {
      command: 'cat-file',
      withArgsTail(...args) {
        return { ...this, args: [ 'blob' ].concat(args) };
      },
    }
    ...
    it('can retrieve the content lines of a file', async function () {
      ...
      }, expectedGitCmd.withArgsTail(`${sharedCasefilesRef}:${casefilePath}`));
      const result = await this.repo.getCasefile(casefilePath);
      expect(result).to.deep.include(content);
    });
    ...
    it('rejects if git exits with an error', async function () {
      ...
      await expect(this.repo.getCasefile(casefilePath))
        .is.rejectedWith(ExtendedError).eventually.with.property('exitCode', 128);
    });
    ...
    it('can retrieve the content lines of a file as it existed before a given commit', async function () {
      ...
      }, expectedGitCmd.withArgsTail(`${parentCommit}:${casefilePath}`));
      const result = await this.repo.getCasefile(casefilePath, {
        beforeCommit: commit,
      });
      expect(result).to.deep.include(content);
  ...
  describe('.prototype.deleteCasefilePaths()', function () {
    ...
    it(`does nothing if all given paths are instances not present in their casefile group`, async function() {
      const casefileName = 'a casefile';
      const instanceId = '2a66a2ea-a400-5e0c-bd26-8365eaaedecc';
      const path = `${casefileName}/${instanceId}`;
      const tree = new GitTree({
  ...
  describe('.prototype.revParse()', function () {
    ...
    it('rejects on an invalid committish (git error exit code)', async function () {
      const committish = sharedCasefilesRef;
      const expectedHash = 'e0de45865af50fd70508636b66bd1ab8249bdf91';
      this.gitMock.expectCall(async ({ stdio: { stdout } }) => {
        return { exitCode: 128 };
    ...
    it(`rejects on an invalid committish (returned by git)`, async function() {
      const committish = sharedCasefilesRef;
      const expectedHash = 'e0de45865af50fd70508636b66bd1ab8249bdf91';
      this.gitMock.expectCall(async ({ stdio: { stdout } }) => {
        await streamConsumed(stdout, () => {
  ...
  describe('.prototype.mktree', function () {
    ...
    it('rejects with code InvalidTreeEntry when slash(es) in the entry names', async function () {
    ...
    it(`rejects with code InvalidTreeResult if git does not output a tree hash`, async function() {
      this.gitMock.expectCall(async ({ stdio: { stdin, stdout } }) => {
        const input = await readAll(stdin);
        expect(input).to.be.a('string');
        expect(input.slice(-1)).to.equal('\0');
      ...
      await expect(this.repo.mktree(entries))
        .is.rejected.eventually.with.property('code', 'InvalidTreeResult')
        ;
    });
  });
  ...
  describe('.prototype.push()', function () {
    ...
    it('can resolve successfully', async function () {
      ...
      await this.repo.push(remote, { source, dest });
    });
    ...
    it('can be told to force-push', async function () {
      ...
      await this.repo.push(remote, { source, dest, force: true });
    });
    
    it('can push a string spec', async function () {
      const branchName = 'a-branch-name';
      this.gitMock.expectCall(async () => {
        
      }, { command: 'push', args: [ remote, `${branchName}:refs/heads/${branchName}` ] });
  ...
  describe('.prototype.getDeletedCasefileRefs()', function () {
    ...
    it('does not require an argument', async function () {
      ...
      expect(result).to.have.deep.members(
        deletionRecords.map(r => ({
          ...r,
          committed: new Date(r.committed),
        }))
      );
    });
    ...
    it(`handles bad git commit header line by keeping same commit hash and date`, async function() {
      const deletionRecords = [
        {
          commit: 'b725fae446a0485746dad8d3bd25dbcbc28b15a7',
          committed: '2022-01-01 07:08:09 -0500',
      ...
      this.gitMock.expectCall(async ({ stdio: { stdout } }) => {
        await streamConsumed(stdout, () => {
          ...
          expect(recs[1].commit).to.equal(recs[2].commit);
          expect(recs[1].committed).to.equal(recs[2].committed);
          stdout.write(`- d79337d618836e0077e756f5d67a569774a9f8fc S@I$SaRs]oe\n`);
          stdout.write(`D\0${recs[2].path}\0`);
          stdout.write('\0');
        });
      }, {
    ...
    it(`rejects with code InvalidGitLogOutput on a commit info line that has no newline sequence`, async function() {
      const deletionRecords = [
        {
          commit: 'b725fae446a0485746dad8d3bd25dbcbc28b15a7',
          committed: '2022-01-01 07:08:09 -0500',
      ...
      this.gitMock.expectCall(async ({ stdio: { stdout } }) => {
        await streamConsumed(stdout, () => {
          ...
          expect(recs[1].commit).to.equal(recs[2].commit);
          expect(recs[1].committed).to.equal(recs[2].committed);
          stdout.write(`- ${recs[1].commit.slice(0, -5)}lmnop ${recs[1].committed}`);
          stdout.write(`D\0${recs[2].path}\0`);
          stdout.write('\0');
        });
      }, {
      ...
      await expect(this.repo.getDeletedCasefileRefs())
        .is.rejected.eventually.with.property('code', 'InvalidGitLogOutput');
    });
  });
  
  describe('.prototype.getBlobContent()', function () {
    ...
    it(`returns the contents of a file in the HEAD commit`, async function() {
      ...
      }, expectedGitCmd.withArgsTail(`HEAD:${path}`));
      const result = await this.repo.getBlobContent(path);
      expect(result).to.deep.equal(content);
    });
    
    it(`returns the contents of a specified commit`, async function() {
      const commit = '67875457ca0157b511ada8215214d5d0bd8cba0c';
      const content = JSON.stringify({
        name: 'example',
        version: '1.0.0',
      });
      ...
      }, expectedGitCmd.withArgsTail(`${commit}:${path}`));
      const result = await this.repo.getBlobContent(path, { commit });
      expect(result).to.deep.equal(content);
    });
  });
  
  describe('.prototype.lineIntroduction()', function () {
    const filePath = 'package.json';
    const line = 42;
    const introduction = {
      commit: 'f7e190fee67189b3b748045bd62fdaeb5d714a1c',
      line: 22,
    };
    
    describe('finding commit and line where the indicated line first appeared', function () {
      test(`from line in file on disk`, async function() {
        this.gitMock.expectCall(async ({ cwd, stdio: { stdout } }) => {
          argumentAssertions(() => {
            expect(cwd).to.equal(dirname(filePath));
          });
          await streamConsumed(stdout, () => {
            stdout.write(`${introduction.commit} ${introduction.line} ${line} 1\n`);
          });
        }, {
          command: 'blame',
          opts: { L: `${line},${line}`, porcelain: true },
          args: [ '--', basename(filePath) ]
        });
        const result = await this.repo.lineIntroduction(filePath, line);
        expect(result).to.deep.equal(introduction);
      });
      
      test(`from line in content (from editor buffer)`, async function() {
        const liveContent = Array.from(
          '01234567890123456789012345678901234567890123456789',
          (c, i) => (i + 1 === line ? 'SOMETHING SPECIAL' : 'nothing of note') + '\n'
        ).join('');
        this.gitMock.expectCall(async ({ cwd, stdio: { stdin, stdout } }) => {
          const providedContent = await readAll(stdin);
          argumentAssertions(() => {
            expect(cwd).to.equal(dirname(filePath));
            expect(providedContent).to.equal(liveContent);
          });
          await streamConsumed(stdout, () => {
            stdout.write(`${introduction.commit} ${introduction.line} ${line} 1\n`);
          });
          ...
          command: 'blame',
          opts: { L: `${line},${line}`, porcelain: true, contents: '-' },
          args: [ '--', basename(filePath) ]
        });
        const result = await this.repo.lineIntroduction(filePath, line, {
          liveContent
      ...
      test(`in specified commit`, async function() {
        const referenceCommit = '3cc96bf37e6249cdf0048e31b6df15063061b460';
        this.gitMock.expectCall(async ({ cwd, stdio: { stdout } }) => {
          argumentAssertions(() => {
            expect(cwd).to.equal(dirname(filePath));
          });
          ...
          command: 'blame',
          opts: { L: `${line},${line}`, porcelain: true },
          args: [ referenceCommit, '--', basename(filePath) ]
        });
        const result = await this.repo.lineIntroduction(filePath, line, {
      ...
      test(`no origin found`, async function() {
        this.gitMock.expectCall(async ({ cwd, stdio: { stdout } }) => {
          argumentAssertions(() => {
            expect(cwd).to.equal(dirname(filePath));
          });
          await streamConsumed(stdout, () => {
            stdout.write(`0000000000000000000000000000000000000000 ${line} ${line} 1\n`);
          });
        }, {
          command: 'blame',
          opts: { L: `${line},${line}`, porcelain: true },
        ...
        await expect(this.repo.lineIntroduction(filePath, line))
          .is.rejectedWith(ExtendedError)
  ...
  describe('.prototype.findCurrentLinePosition()', function () {
    const lineInContent = 42;
    const filePath = 'package.json';
    const peg = {
      commit: '80493581658bb28ac45860ea1704fc6e721293ca',
      line: 27
    };
    const content = Array.from(
      '01234567890123456789012345678901234567890123456789',
      (c, i) => (i === lineInContent ? 'SOMETHING SPECIAL' : 'nothing of note') + '\n'
    ).join('');
    const expectedGitCmd = {
      command: 'blame',
      ...
      omitOpts(...keys) {
        ...
        for (const key of keys) {
          delete resultOpts[key];
        }
        return { ...this, opts: resultOpts };
      },
    };
    
    it(`can return a line`, async function() {
      this.gitMock.expectCall(async ({ cwd, stdio: { stdin, stdout } }) => {
        const providedContent = await readAll(stdin);
        argumentAssertions(() => {
          expect(cwd).to.equal(dirname(filePath));
          expect(providedContent).to.equal(content);
        });
        await streamConsumed(stdout, () => {
          stdout.write(`${peg.commit} ${peg.line} ${lineInContent} 1\n`);
        });
      }, expectedGitCmd);
      const result = await this.repo.findCurrentLinePosition(
        filePath,
        peg,
        content
      );
      expect(result).to.deep.equal({ line: lineInContent });
    });
    
    it(`rejects with code LineNotFound if line is not found`, async function() {
      this.gitMock.expectCall(async ({ cwd, stdio: { stdin, stdout } }) => {
        const providedContent = await readAll(stdin);
        argumentAssertions(() => {
          expect(cwd).to.equal(dirname(filePath));
      ...
      await expect(this.repo.findCurrentLinePosition(
        filePath,
        peg,
        content
      )).is.eventually.rejectedWith(ExtendedError).which.includes({
        code: 'LineNotFound',
        file: filePath,
        ...peg,
      });
    ...
    it(`ignores blame output lines about other commits`, async function() {
      this.gitMock.expectCall(async ({ cwd, stdio: { stdin, stdout } }) => {
        const providedContent = await readAll(stdin);
        argumentAssertions(() => {
          expect(cwd).to.equal(dirname(filePath));
        ...
        await streamConsumed(stdout, () => {
          stdout.write(`2d17d880a7e48dbff8572326366180e26a79b121 35 117 1\n`);
          stdout.write(`${peg.commit} ${peg.line} ${lineInContent} 1\n`);
        });
      }, expectedGitCmd);
      const result = await this.repo.findCurrentLinePosition(
    ...
    it(`ignores blame output lines about other changes in the same commit`, async function() {
      this.gitMock.expectCall(async ({ cwd, stdio: { stdin, stdout } }) => {
        const providedContent = await readAll(stdin);
        argumentAssertions(() => {
          expect(cwd).to.equal(dirname(filePath));
    ...
    it(`can run against the current file content`, async function() {
      this.gitMock.expectCall(async ({ cwd, stdio: { stdin, stdout } }) => {
        argumentAssertions(() => {
          expect(cwd).to.equal(dirname(filePath));
        });
      ...
      }, expectedGitCmd.omitOpts('contents'));
      const result = await this.repo.findCurrentLinePosition(
        filePath,
        peg,
      );
  ...
  describe('(private)', function () {
    describe('.prototype.findLatestCommitParentWithPath()', function () {
      ...
      it(`ignores a parent that responds with an error about commit and date`, async function() {
        const mostRecentParentCommit = 'f771b18d9312f8970e8085f745fddbfc49d8a8d4';
        const BAD_RESULT = Symbol('bad result');
        const parentCommits = {
          [ mostRecentParentCommit ]: 7926077775151104,
          'fa37d436f3634f4910ccb8f925785275ac5f7a90': BAD_RESULT,
        };
        this.repo.mockMethod('getDateOfLastChange', async (path, { commit }) => {
          expect(path).to.equal(casefilePath);
          if (!parentCommits.hasOwnProperty(commit)) {
          ...
          if (result === BAD_RESULT) {
            throw new Error('as if git call failed');
          }
          parentCommits[commit] = null;
          return result;
        });
    ...
    describe('.prototype.commitCasefilesTree()', function () {
      ...
      it(`rejects with code InvalidCommit if git does not output a commit hash`, async function() {
        this.gitMock.expectCall(async ({ stdio: { stdout } }) => {
          await streamConsumed(stdout, () => {
            stdout.write('\n');
          });
        }, { ...expectedGitCmd, args: [ tree ] });
        await expect(this.repo.commitCasefilesTree(tree, { message }))
          .is.rejected.eventually.with.property('code', 'InvalidCommit');
      });
    });
  });
});
